...
# db1_rep = pymysql.connect(host='localhost', user='root', password='hhwswhrO1209', database='ratemybeer1_rep')
# db2_rep = pymysql.connect(host='localhost', user='root', password='hhwswhrO1209', database='ratemybeer2_rep')
#
...
#     if db_name == 'db1':
#         # insert into db1
#         cursor1.execute("INSERT INTO Review (review_id, beer_id, review_text, rating, create_time, update_time) VALUES (%s, %s, %s, %s, NOW(), NOW())",
#                         (review_id, beer_id, review_text, rating))
#         db1.commit()
#         return jsonify({"message": "Review added into db1 successfully"})
#     else:
#         # insert into db2
#         cursor2.execute("INSERT INTO Review (review_id, beer_id, review_text, rating, create_time, update_time) VALUES (%s, %s, %s, %s, NOW(), NOW())",
#                         (review_id, beer_id, review_text, rating))
#         db2.commit()
...
def add_review():
    ...
    # # Insert into the appropriate database based on the hash
    # if db_name == 'db1':
    #     # insert into db1 and db1 replica
    #     cursor1.execute("INSERT INTO Review (review_id, beer_id, review_text, rating, create_time, update_time) VALUES (%s, %s, %s, %s, NOW(), NOW())",
    #                     (review_id, beer_id, review_text, rating))
    #     db1.commit()
    #     cursor3.execute("INSERT INTO Review (review_id, beer_id, review_text, rating, create_time, update_time) VALUES (%s, %s, %s, %s, NOW(), NOW())",
    #                     (review_id, beer_id, review_text, rating))
    #     db1_rep.commit()
    #     return jsonify({"message": "Review added into db1 and db1 replica successfully"})
    # else:
    #     # insert into db2 and db2 replica
    #     cursor2.execute("INSERT INTO Review (review_id, beer_id, review_text, rating, create_time, update_time) VALUES (%s, %s, %s, %s, NOW(), NOW())",
    #                     (review_id, beer_id, review_text, rating))
    #     db2.commit()
    #     cursor4.execute("INSERT INTO Review (review_id, beer_id, review_text, rating, create_time, update_time) VALUES (%s, %s, %s, %s, NOW(), NOW())",
    #                     (review_id, beer_id, review_text, rating))
    #     db2_rep.commit()
    #     return jsonify({"message": "Review added into db2 and db2 replica successfully"})
    # Insert into the appropriate database based on the hash
    if db_name == 'db1':
...
#         values.append(filter_data['rating'])
#
#     if conditions:
...
#         print(sql_query)
#
#         # execute queries for both databases
#         cursor1.execute(sql_query, values)
#         results_db1 = cursor1.fetchall()
#
#         cursor2.execute(sql_query, values)
...
#         data = [{'review_id': row[0], 'beer_id': row[1], 'beer_name': row[2], 'beer_type': row[3], 'review_text': row[4], 'rating': row[5]} for row in merged_results]
#         return jsonify(data)
#
...
#     assigned_db = hash_uuid(review_id)
#
#     # Update review in primary and replica databases
#     if assigned_db == "db1":
#         primary_db_connection = db1
#         replica_db_connection = db1_rep
#         db_name = "db1"
#     else:
#         primary_db_connection = db2
#         replica_db_connection = db2_rep
#         db_name = "db2"
#
#     primary_cursor = primary_db_connection.cursor()
#     replica_cursor = replica_db_connection.cursor()
#
#     # construct SQL
#     update_query = "UPDATE Review SET "
#     update_values = []
...
#     update_values.append(review_id)
#
#     # execute update on primary database
#     primary_cursor.execute(update_query, update_values)
#     primary_db_connection.commit()
#
#     # execute update on replica database
#     replica_cursor.execute(update_query, update_values)
#     replica_db_connection.commit()
#
...
#         primary_cursor = primary_db_connection.cursor()
#     except pymysql.err.OperationalError as e:
#         return jsonify({"error": f"Failed to create cursor for {db_name}: {e}"}), 500
#
#     try:
#         replica_cursor = replica_db_connection.cursor()
#     except pymysql.err.OperationalError as e:
#         return jsonify({"error": f"Failed to create cursor for {db_name}_rep: {e}"}), 500
#
#     # construct SQL
#     update_query = "UPDATE Review SET "
#     update_values = []
...
#         primary_db_connection.commit()
#     except pymysql.err.OperationalError as e:
#         return jsonify({"error": f"Failed to update review in {db_name}: {e}"}), 500
#         pass
#
#     # execute update on replica database
#     try:
...
#         replica_db_connection.commit()
#     except pymysql.err.OperationalError as e:
#         return jsonify({"error": f"Failed to update review in {db_name}_rep: {e}"}), 500
#     return jsonify({"message": f"Review updated successfully in {db_name} and its replica"})
...
#
#         # Execute queries for db1 (primary) and db1_rep (replica)
#         for cursor, db_name in [(cursor1, 'db1'), (cursor3, 'db1_rep')]:
#             try:
#                 # construct SQL
#                 sql_query = "SELECT Review.review_id, Review.beer_id, beer.beer_name, beer.beer_type, Review.review_text, Review.rating FROM Review INNER JOIN beer ON Review.beer_id = beer.beer_id"
#
...
#
#         # Execute queries for db2 (primary) and db2_rep (replica)
#         for cursor, db_name in [(cursor2, 'db2'), (cursor4, 'db2_rep')]:
#             try:
#                 # construct SQL
#                 sql_query = "SELECT Review.review_id, Review.beer_id, beer.beer_name, beer.beer_type, Review.review_text, Review.rating FROM Review INNER JOIN beer ON Review.beer_id = beer.beer_id"
#
...
#                 filtered_results = [row for row in results if row[0] not in unique_review_ids]
#
#                 # Add filtered results to part 2
...
#                 print(f"Failed to execute query on {db_name}: {e}")
#
#         # Combine part 1 and part 2 results
