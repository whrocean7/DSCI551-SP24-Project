...
# def get_db_connection(db_name="ratemybeer1"):
#     if db_name == "db1":
#         return pymysql.connect(host='localhost', user='root', password='hhwswhrO1209', database='ratemybeer1')
#     else:
#         return pymysql.connect(host='localhost', user='root', password='hhwswhrO1209', database='ratemybeer2')
...
#     try:
#         cursor.execute("SELECT beer_id FROM beer WHERE beer_name = %s AND beer_type = %s", (beername, beertype))
#         beer_id = cursor.fetchone()
#         if not beer_id:
#             return jsonify({"error": "Beer not found"}), 404

#         review_id = str(uuid.uuid4())
#         cursor.execute("INSERT INTO Review (review_id, beer_id, review_text, rating) VALUES (%s, %s, %s, %s)", (review_id, beer_id[0], review_text, rating))
#         conn.commit()
#     finally:
#         cursor.close()
#         conn.close()
...
# def update_review():
#     data = request.json
#     review_id, review_text, rating = data.get('review_id'), data.get('review_text'), data.get('rating')
#     if not review_id:
#         return jsonify({"error": "review_id is required"}), 400

#     conn = get_db_connection(hash_uuid(review_id))
#     cursor = conn.cursor()
#     try:
#         cursor.execute("UPDATE Review SET review_text = %s, rating = %s WHERE review_id = %s", (review_text, rating, review_id))
#         conn.commit()
#     finally:
#         cursor.close()
#         conn.close()
...
#     reviews = [{'review_id': r[0], 'beer_id': r[1], 'beer_name': r[2], 'beer_type': r[3], 'review_text': r[4], 'rating': r[5]} for r in results]
